...
\section{Cilji}
Cilj za projektno nalogo, nam je bil narediti program, v katerem se vsak uporabnik (tudi tak 
ki ne pozna izvorne kode) lahko vidi razliko med različnimi prostori ki niso evklidski, in 
jih primerja z evklidskim prostorom. Želeli smo si, da bi bilo dodajanje novih prostorov in 
...
\begin{enumerate}
  ...
  rotiramo \(\theta += \Delta x \) veliko bolj oddaljeni, kot ko je \( \phi \) blizu \( \pi/2 \) 
...
\end{enumerate}
Postopek se zdi kar zapleten, vendar je proizvedel bolj natančne rezultate kot druge metode (npr. mreža točk 
za piksle nekoliko pred kamero, kjer za vsak piksel generiraš žarek med kamero in točko na mreži), ko 
kamera ne gleda naravnost. Omogoča nam tudi določanje širine kadra v stopinjah, kar je zelo uporabno.
...
Na sceni kamera gleda v krogle obrnjena za 10 stopinj navzgor (da je kader bolj unikaten). Enaka scena, 
...
Pomembno je omeniti tudi, da v primeru velikih radijev sfer po katerih se premikajo žarki prostor zgleda manj popačeno. 
Scena ki smo jo videli že prej v prostoru 2sphere kjer je radij sfer 25 zgleda skoraj kot da kamero usmerili v tla. To je 
ponovno posledica dejstva, da objekti pred nami v sliko vstopajo od zgoraj. Opazimo pa tudi, da je krogla na sliki 
...
Da ne bi bilo potrebno na vsako sliko čakati predolgo časa, so se zauzeli da algoritme pohitrimo. 

\subsection{Pohitritev osnovnega algoritma}
Po krajšem razmisleku, odkrijemo naslednji težavi: 
\begin{enumerate}
  \item Korakanje žarka po prostoru hitro postane računsko prezahtevno, če je korak majhen.
  \item Če je korak pre velik, se lahko zgodi, da v v neki točki preskoči presečišče z objektom.
\end{enumerate}

\begin{figure} [H]
  \centering
  \includegraphics[width=0.6\linewidth]{Images/step_size_issue.png}
  \caption{Zaradi pre velikega koraka, zgrešimo presečišče.}
...
\( s(t, T, \vec{v}) = [x, y, z] \) (ali pa kar \( s(t) \) ker se pogovarjamo o enem specifičnem žarku)
 pa je funkcija, ki vrne točko v prostoru kjer se nahaja žarek z izhodiščem v \( T_0\) in smerjo 
\( \vec{v} \) ob času \( t \).
...
kamere (vrednost \( t \)) v evklidskem prostoru in prostoru 2sphere. (Obkrožen je tudi piksel katerega žarek spremljamo, 
v sredini slike)
...
Hitro nam postane jasno, da razdalja žarka sledi funkciji, ki nima, ima eno, ali pa dve ničli. 
(za 2sphere, seveda iščemo le na intervalu \(t \in [0, 2\pi] \)). Imamo le en minimum, 
za katerega velja da če je večji od 0, se žarek objekta nikoli nebo dotaknil.
...
\begin{itemize}
  ...
  \item Kdokler se funkcija \( o(t) \) NE povečuje, povečujemo korak 
  (da hitreje pridemo do presečišča)
\end{itemize}
...
Flat torus uporablja kar formulo za presečišče iz evklidskega prostora, le da še preslika
žarke če naletijo na rob torusa.

\subsection{Pohitritev iskanja presečišč za 2Sphere}

Direktna izpeljava formule za presečišče žarka z 2Sphere je precej bolj zapletena,
zato mi ni uspela. Vseeno pa smo našli rešitev, ki je hitrejša od iterativnega pristopa
računanja odvoda odvodov in geodetk.

Vsak žarek, gre namreč čez vrh in dno sfere. Zato ga lahko parametriziramo, tako kot bi 
parametrizirali točko na sferi:
\begin{equation}
  \begin{split}
    &x = R\cos(v)\cos(t) + x_0 \\
    &y = R\sin(t) + y_0 \\
...
Radij krogle \( R \) je podan parameter prostora.
\(v\) lahko izračunamo iz smernega vektorja žarka v, kot:
\begin{equation}
  \begin{split}
  &v = \arctan \left( \frac{v_z}{v_x} \right) \\
  &u = \arctan \left( \frac{v_y}{\sqrt{v_x^2 + v_z^2}} \right)
  \end{split}
\end{equation}

Nekaj imamo še dela, da poiščemo center krogle \( [x_0, y_0, z_0] \). Ker nas zanima le  
najnižja možna krogla v katero lahko žarek vstopi, do centra lahko pridemo kar tako, da 
gremo od kamere kar za \( R\) v smeri vektorja \( v - \pi/2 \).

Ta parametrizacija je povsem kompatibilna z pohitrenim algoritmom za iskanje presečišč, 
ki sem ga opisal prej.
...
prostoru celo manj kot 10 sekund (za 1500 x 1000 sliko).

Sicer obstaja še marsi katea optimizacija, ter še marsi katera funkcionalnost, ki bi jo lahko dodali,
na primer možnosti dodajanja materialov in senčenja. Na žalost nam je za to zmanjkalo časa.

Seveda je bil eden izmed ciljev tudi, enostavno dodajanje novih objektov in prostorov. Z veseljem lahko 
povem, da nam je tudi to uspelo:
