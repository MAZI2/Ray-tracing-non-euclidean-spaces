...
Želeli smo si splošen način, kako lahko premikamo žarek skozi katerikoli prostor. Tako smo zasnovali algoritem, ki temelji na iskanju ničle funkcije vsakega posameznega objekta $o = (x, y, z)$, ko v njo vstavimo
pozicijo žarka ob podanem "času" $t$. Algoritem torej išče parameter $t$ za katerega velja:
$$f = o(s(t)) = 0$$
...
\item Če se v neki točki funkciji predznak, vemo da smo v tem koraku vstopili v objekt. Razpolovimo korak in ga ponovimo.
...
\item Postopek razpolavljanja ponavljamo, kdokler korak ni manjši kot $\varepsilon$.
\begin{figure}[H]
    \centering
...
%$\vec_{r}_{t}$ pa krajevni vektor točke presečišča. TODO error metal kle
...
y &= \left( \text{y} + \text{korak} \cdot \text{smer y} \right) \mod 1 \\ %TODO Kle a b c?
...
Funkciji \textbf{signs} in \textbf{checkIntersect} implementirata funkcije $f$, \textbf{distance} pa implementira formuli \eqref{e:distSphere} in %TODO A tukej smo zdj opisval funkcije iz mathlaba al iz pythona? Ker v pythonu nimam teh funkcij zares vec..
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.5\linewidth]{Images/flat_torus.png} %TODO A damo uno sliko kle k je več repetitionov?
