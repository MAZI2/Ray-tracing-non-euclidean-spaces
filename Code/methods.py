...
def adaptive_step_broyden(ray: Ray, object: _IntersectableObject, space:  _Space,
    ...
    t = np.array([t0, t0+step_size], dtype=np.float64)
    y = np.array([object.equation(space.xyz_equation(ray, t0)), 
                  object.equation(space.xyz_equation(ray, t0+step_size))], dtype=np.float64)

    for _ in range(max_iter):
        # If we are going towards the object, run broydens method
        ...
        if y[1] > y[0]: 
            ...
            if step_size < tollerance: return y[1] # We hit the object
            step_size /= 2
            continue
        elif t[1] > max_t: 
        ...
        t[1] = t[1] + step_size
        y[0] = y[1]
        y[1] = object.equation(space.xyz_equation(ray, t[1]))
